...
* **Current state:** Kubernetes-native batch schedulers that support gang scheduling include the coscheduling plugin (via PodGroups), Armada, KAI Scheduler, Kueue, and Volcano. Native gang scheduling with the Workloads API has been implemented in Kubernetes 1.35 as an alpha feature, with a goal to reach beta in Kubernetes 1.36.
