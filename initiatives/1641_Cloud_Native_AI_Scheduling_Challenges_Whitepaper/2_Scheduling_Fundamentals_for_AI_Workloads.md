...
Building this into the core Kubernetes scheduler would add complexity that most users do not need. Despite Kubernetes having a pluggable scheduler framework, various workloads (high-throughput computing, for example) demand more than can be achieved with a scheduler plugin approach alone. Instead, the ecosystem has developed meta-schedulers and queue managers that use well-defined extensions to sit alongside or extend the default scheduler.
