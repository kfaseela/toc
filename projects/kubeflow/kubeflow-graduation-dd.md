...
- Kubeflow's [GitOps-based access control](https://github.com/kubeflow/internal-acls) via Peribolos and Prow is particularly well-designed: org membership, team membership, and per-repo permissions are all declared in a versioned YAML file and continuously reconciled against GitHub, providing auditable and transparent access enforcement with 2FA requirements.
...
- The Kubeflow maintainers and community have been exceptionally responsive throughout this review. Multiple governance issues, documentation gaps, and recommendations raised during the DD were addressed promptly — PRs were merged, issues were filed and assigned, and maintainers engaged constructively on all feedback. This level of responsiveness reflects a healthy, active community that takes quality and graduation readiness seriously.
...
The November 2024 presentation covered project overview, 2024 highlights, a live fine-tuning demo, and explicitly named graduation as a 2025/2026 goal, citing adoption and security as focus areas. No concerns or action items were raised by TAG Runtime.
...
All 6 graduation subprojects have dedicated documentation pages on kubeflow.org (last updated May 2026), with installation guides, user guides, operator guides, and reference documentation. The [kubeflow.org docs site](https://www.kubeflow.org/docs/) is comprehensive and actively maintained. The [General Technical Review](https://github.com/kubeflow/community/blob/master/KUBEFLOW-GENERAL-TECHNICAL-REVIEW.md) (GTR, 965 lines) provides an extensive technical reference document. Kubeflow Notebooks has an architectural dependency on Kubeflow Central Dashboard and cannot be deployed standalone — this is a known design characteristic, not a documentation gap.
...
Decision-making is documented across three processes: (1) KSC decisions ([committee-steering/charter.md](https://github.com/kubeflow/community/blob/master/committee-steering/charter.md)) covering Normal Decisions (quorum + majority) and Special Decisions (70% threshold with per-org weighting) for governance/charter changes; (2) [KEP process](https://github.com/kubeflow/community/tree/master/proposals) for user/operator-facing or major architectural changes; (3) [Ecosystem join process](https://github.com/kubeflow/community/tree/master/ecosystem) for external project partnerships (KSC vote required). All processes are documented with templates and lifecycle states.
...
The [contributor ladder](https://www.kubeflow.org/docs/about/membership/) defines the full lifecycle: onboarding criteria for each role (Member → Reviewer → Approver), offboarding (12-month inactivity → org removal; emeritus_approvers field for inactive approvers), and WG Lead/Chair removal via super-majority vote. The `emeritus_approvers` field was formally introduced in February 2025 and is in active use across core subproject OWNERS files.
...
Kubeflow uses a GitOps ACL system ([kubeflow/internal-acls](https://github.com/kubeflow/internal-acls)) via Peribolos and Prow. `github-orgs/kubeflow/org.yaml` declares all org membership, team membership, and repo-level permissions, reconciled continuously against GitHub. The KSC team, WG lead teams, and security-team are all declared in org.yaml and match the governance documents. Per-repo code ownership is enforced via OWNERS files (Prow `lgtm` + `approve` requirements). `default_repository_permission: read` enforces least privilege.
...
**Add process:** Documented 8-step process in [community/subprojects/README.md](https://github.com/kubeflow/community/tree/master/subprojects): GitHub issue → Google Doc proposal → community call demo → PR to subprojects/ directory (using template) → KSC vote → repo transfer + migration checklist. Prior applications (spark-operator, model-registry) are archived in the directory. Directly discoverable from the [kubeflow.org introduction page](https://www.kubeflow.org/docs/started/introduction/#kubeflow-subprojects) (confirmed 2026-06-17).
...
LF Insights (past 365 days, as of 2026-06-11): 1,174 active contributors from 177 organizations, +40.6% YoY growth, 49% quarterly contributor retention (rated "excellent"). Recent maintainer additions across multiple subprojects demonstrate active recruitment (trainer#2659, pipelines#12059, hub#1153 — all merged 2025–2026, each with explicit criteria verification). Note: the graduation application cited ~1,500 contributors / ~280 orgs — the LF Insights snapshot shows 1,174 / 177, likely due to different time-window definitions; the trend direction is consistent and positive.
...
Kubeflow uses a GitOps ACL system via [kubeflow/internal-acls](https://github.com/kubeflow/internal-acls): all org membership and repo permissions are declared in `github-orgs/kubeflow/org.yaml` and continuously reconciled by Peribolos. 2FA is a documented requirement for all Kubeflow GitHub org members ([contributor ladder](https://www.kubeflow.org/docs/about/membership/)). `default_repository_permission: read` enforces least privilege across all repos.

- [X] **Document assignment of security response roles and how reports are handled.**

Security response roles are documented in each subproject's security policy: reports are handled by "Kubeflow project owners" with KSC (`ksc@kubeflow.org`) as the escalation channel. A dedicated `security-team` is declared in `internal-acls/org.yaml`. The disclosure process (acknowledgment, investigation, resolution, reporter notification, public disclosure) is documented consistently across all 6 subproject policies.
...
- [Pipelines](https://github.com/kubeflow/pipelines/blob/master/ADOPTERS.md): 4 adopters ⚠️ sparse for the most widely deployed subproject
- [Notebooks](https://github.com/kubeflow/notebooks/blob/main/ADOPTERS.md): 1 adopter (Red Hat only) ⚠️ very thin — CERN addition in progress: [Add CERN to Notebooks ADOPTERS.md (notebooks#1190)](https://github.com/kubeflow/notebooks/pull/1190)

Notebooks and Pipelines ADOPTERS.md files are notably sparse relative to their reported download/deployment metrics. This is tracked as an observation — adopter lists are self-reported and incomplete; the 9M downloads/month signal and adopter interview evidence provide independent validation.
